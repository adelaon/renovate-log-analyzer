...
func ProcessLogFile(logFilePath, simpleReason string) (string, *SimpleReport, error) {
	...
	if _, err := os.Stat(logFilePath); os.IsNotExist(err) {
		return simpleReason, report, fmt.Errorf("log file does not exist: %s", logFilePath)
	}

	// Open and read the file
	file, err := os.Open(logFilePath)
	if err != nil {
		return simpleReason, report, fmt.Errorf("failed to open log file: %w", err)
	}
	defer file.Close()
	...
	if err := scanner.Err(); err != nil {
		return simpleReason, report, fmt.Errorf("error reading log file: %w", err)
	}
